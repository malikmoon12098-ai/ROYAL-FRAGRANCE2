...
const AppState = {
    ...
    async onUserLoggedIn(user) {
        ...
        // Setup check on every route change (Removed redirection to setup)
        window.addEventListener('hashchange', () => {
            // No longer forcing #setup
        });

        // Initial check
        const settings = Store.getSettings();
        if (!settings.setupFinished) {
            // Automatically setup as Perfume Business if not already done
            Store.saveSettings({
                businessName: 'PERFUME BUSINESS',
                phone: '',
                owner: 'Admin',
                currency: 'Rs',
                setupFinished: true
            });
            console.log('Default Perfume Business settings applied');
        }
    }
};
