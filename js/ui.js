...
const UI = {
    ...
    pages: {
        ...
        dashboard: () => {
            ...
            // Calculate Totals
            const toCollect = partiesList.reduce((sum, p) => sum + (Number(p.balance) || 0), 0);
                ...
                <div class="space-y-6">
                    ...
                    <div class="bg-cardBg rounded-3xl p-6 text-white shadow-xl border border-white/5 relative overflow-hidden group">
                        ...
                        <div class="grid grid-cols-2 gap-4 relative z-10">
                            ...
                            <div class="space-y-1">
                                <div class="text-[10px] font-bold text-rose-400 uppercase tracking-tighter">${i18n.t('toCollect')}</div>
                                <div class="text-2xl font-black text-white">${UI.formatMoney(toCollect)}</div>
                            </div>
                        </div>
                    </div>
                    ...
                    </div>                    <!-- Quick Navigation Grid (Vibrant Style) -->
                    <div class="grid grid-cols-2 gap-4">
...
`;
                    ...
                    <div class="bg-cardBg rounded-xl shadow-sm border border-white/5 overflow-hidden">
                        ...
                        ` : `
                            <div class="divide-y divide-white/5">
                                ${invoices.map(inv => `
                                    <div class="p-5 hover:bg-white/5 flex justify-between items-center group transition-colors cursor-pointer" onclick="UI.hooks.viewInvoiceDetail('${inv.id}')">
                                        <div class="flex items-center gap-4">
                                            ...
                                            <div>
                                                <div class="font-bold text-white">${inv.partyName || i18n.t('cashSale')}</div>
                                                <div class="text-[10px] text-slate-400 font-medium uppercase tracking-wider">Bill #${inv.id.slice(-6).toUpperCase()} • ${new Date(inv.date).toLocaleDateString()}</div>
                                            </div>
                                        </div>
                                        <div class="text-right flex flex-col items-end gap-1">
                                            <div class="font-black text-white text-lg">${UI.formatMoney(inv.total)}</div>
                                            <div class="text-[10px] font-black ${inv.due > 0 ? 'text-red-500' : 'text-emerald-500'} uppercase">
                                                ${inv.due > 0 ? 'Due: ' + UI.formatMoney(inv.due) : 'Paid Full'}
                                            </div>
                                            <button onclick="event.stopPropagation(); UI.hooks.deleteInvoice('${inv.id}')" class="w-8 h-8 rounded-lg bg-red-500/10 text-red-500 flex items-center justify-center hover:bg-red-500 hover:text-white transition-all opacity-0 group-hover:opacity-100 shadow-sm mt-1">
                ...
                <div class="h-full flex flex-col">
                    ...
                    <div class="flex-1 overflow-y-auto space-y-4 pb-24">
                        <div class="bg-cardBg p-5 rounded-3xl shadow-sm border border-white/5">
                            <label class="block text-xs font-black text-slate-500 uppercase tracking-tighter mb-2">Customer Name (Optional)</label>
                            <div class="flex gap-2">
                                <input type="text" id="saleCustomerName" placeholder="Enter Customer Name" class="w-full p-4 rounded-xl bg-appBg border-none font-bold text-gray-200 focus:ring-2 focus:ring-primary/20" list="existingPartiesList">
                                <datalist id="existingPartiesList">
                                    ${parties.map(p => `<option value="${p.name}">`).join('')}
                                </datalist>
                                <div class="hidden">
                                    <input type="hidden" id="saleParty" value="">
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="bg-cardBg p-5 rounded-3xl shadow-sm border border-white/5">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4">
                                <div class="md:col-span-6">
                                    <input type="text" id="itemSelectName" placeholder="Product Name" class="w-full p-3 rounded-xl bg-appBg border-none focus:ring-2 focus:ring-primary/20 font-bold text-gray-200" list="saleProductsList" onchange="const p = Store.getProducts().find(x => x.name === this.value); if (p) { document.getElementById('itemPrice').value = p.price; document.getElementById('itemQty').focus(); }">
                                    <datalist id="saleProductsList">
                                        ${products.map(p => `<option value="${p.name}" data-id="${p.id}" data-price="${p.price}">`).join('')}
                                    </datalist>
                                </div>
        ...
        dues: () => {
            ...
            return `
                <div class="space-y-4 pb-32">
                    ...
                    <div class="bg-cardBg rounded-[2rem] shadow-sm border border-white/5 overflow-hidden">
                        ...
                        ` : `
                            <div class="divide-y divide-white/5">
                                ${parties.map(p => `
                                    <div class="p-5 flex justify-between items-center group hover:bg-white/5 transition-colors cursor-pointer" onclick="UI.hooks.openQuickReceiveFromList('${p.id}')">
                                        <div class="flex items-center gap-4">
                ...
                ${UI.components.modal('quickReceiveModal', 'Udhaar Wasooli', `
                    <form id="quickReceiveForm" class="space-y-4" onsubmit="event.preventDefault(); UI.hooks.processQuickReceive();">
                        <div class="grid grid-cols-1 gap-4">
                            <div class="bg-white/5 p-4 rounded-2xl border border-white/5">
                                <div class="text-slate-400 text-xs font-bold uppercase mb-1">Total Udhaar</div>
                                <div id="quickTotalDue" class="text-2xl font-black text-white">Rs 0</div>
                            </div>
                            <div>
                                <label class="block text-sm font-bold text-gray-200 mb-1.5">Kitne Paise Mile?</label>
                                <div class="relative">
                                    <span class="absolute left-4 top-3.5 text-slate-500 font-bold">Rs</span>
                                    <input type="number" id="quickAmount" required class="w-full pl-12 pr-4 py-4 rounded-xl border border-white/10 bg-white text-black text-xl font-black focus:ring-2 focus:ring-primary/20 outline-none">
                                </div>
                            </div>
                        </div>
                        <button type="submit" class="w-full py-5 bg-emerald-600 text-white rounded-2xl font-black shadow-lg shadow-emerald-900/40 mt-4 active:scale-95 transition-all text-lg">
                            RECEIVE & SAVE
                        </button>
                    </form>
                `)}
        ...
        partyLedger: () => {
            const pid = UI.currentPartyId;
            const party = Store.getParties().find(p => p.id === pid);
            if (!party) return `<div>Party not found</div>`;

            const invoices = Store.get('hk_invoices').filter(i => i.partyId === pid);
            ...
            return `
                <div class="space-y-4 pb-32">
                    <div class="flex items-center gap-3">
                        ...
                        <h2 class="text-2xl font-bold text-white">${party.name}</h2>
                    </div>

                    <div class="bg-cardBg p-6 rounded-[2.5rem] border border-white/5 flex justify-between items-center shadow-xl">
                        <div>
                            <div class="text-slate-500 text-[10px] font-black uppercase tracking-widest mb-1">Current Balance</div>
                            <div class="text-3xl font-black ${party.balance >= 0 ? 'text-emerald-500' : 'text-red-500'}">${UI.formatMoney(Math.abs(party.balance))}</div>
                            <div class="text-[10px] font-bold text-slate-400 mt-1 uppercase">${party.balance >= 0 ? 'Receivable (Lene hain)' : 'Payable (Dene hain)'}</div>
                        </div>
                        <button onclick="UI.hooks.openQuickReceiveModal()" class="bg-emerald-600 text-white px-6 py-3 rounded-2xl font-black shadow-lg shadow-emerald-900/20 active:scale-95 transition-all">
                            JAMMA KAREIN
                        </button>
                    </div>

                    <div class="bg-cardBg rounded-[2rem] border border-white/5 overflow-hidden">
                        <div class="px-6 py-4 border-b border-white/5 bg-white/5">
                            <h3 class="text-xs font-black text-white uppercase tracking-widest">Transaction History</h3>
                        </div>
                        ${invoices.length === 0 ? `<div class="p-10 text-center text-slate-500">No transactions yet</div>` : `
                            <div class="divide-y divide-white/5">
                                ${invoices.reverse().map(inv => `
                                    <div class="p-5 flex justify-between items-center hover:bg-white/5 transition-colors">
                                        <div>
                                            <div class="font-bold text-white">${inv.type === 'SALE' ? 'Sales Bill' : 'Payment Received'}</div>
                                            <div class="text-[10px] text-slate-500 font-bold uppercase tracking-tighter">${new Date(inv.date).toLocaleDateString()}</div>
                                        </div>
                                        <div class="text-right">
                                            <div class="font-black ${inv.type === 'SALE' ? 'text-white' : 'text-emerald-500'}">${inv.type === 'SALE' ? '' : '+'}${UI.formatMoney(inv.total)}</div>
                                            <div class="text-[10px] text-slate-500 font-bold uppercase">${inv.type === 'SALE' ? 'Debit' : 'Credit'}</div>
                                        </div>
                                    </div>
                                `).join('')}
                            </div>
                ...
                ${UI.components.modal('quickReceiveModal', 'Jamma Karein (Receive)', `
                    <form id="quickReceiveForm" class="space-y-4" onsubmit="event.preventDefault(); UI.hooks.processQuickReceive();">
                         <div>
                            <label class="block text-sm font-bold text-gray-200 mb-1.5">Amount Receiving Now</label>
                            <input type="number" id="quickAmount" required min="1" class="w-full px-4 py-3 rounded-xl border border-white/10 bg-white text-black font-black text-lg focus:ring-2 focus:ring-primary/20 outline-none">
                        </div>
                        <button type="submit" class="w-full py-4 bg-emerald-600 text-white rounded-2xl font-black shadow-lg shadow-emerald-900/20 mt-4 active:scale-95 transition-all">
                            CONFIRM RECEIVE
                        </button>
                    </form>
                `)}
                        ...
                        ` : `
                            <div class="divide-y divide-gray-50">
                                ${summaryPurchases.map(pur => `
                                    <div class="p-5 flex justify-between items-center group hover:bg-white/5 transition-colors cursor-pointer" onclick="UI.hooks.viewPurchaseDetail('${pur.id}')">
                                        <div class="flex items-center gap-4">
                ...
                <div class="space-y-4 pb-32">
                    ...
                    <div class="bg-cardBg p-6 rounded-[2rem] shadow-sm border border-white/5 space-y-5">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-black text-slate-500 uppercase tracking-tighter mb-2">Product Name</label>
                                <input type="text" id="purProductName" placeholder="Enter Product Name" class="w-full p-4 rounded-xl bg-appBg border-none font-bold text-gray-200 focus:ring-2 focus:ring-primary/20" list="existingProductsList">
                                <datalist id="existingProductsList">
                                    ${products.map(p => `<option value="${p.name}">`).join('')}
                                </datalist>
                            </div>
                            <div class="grid grid-cols-3 gap-3">
                                <div>
                                    <label class="block text-xs font-black text-slate-500 uppercase tracking-tighter mb-2">Qty</label>
                                    <input type="number" id="purQty" value="1" class="w-full p-4 rounded-xl bg-appBg border-none font-bold text-gray-200 focus:ring-2 focus:ring-primary/20">
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-slate-500 uppercase tracking-tighter mb-2">Cost (Khareed)</label>
                                    <input type="number" id="purCost" placeholder="Cost" class="w-full p-4 rounded-xl bg-appBg border-none font-bold text-gray-200 focus:ring-2 focus:ring-primary/20">
                                </div>
                                <div>
                                    <label class="block text-xs font-black text-slate-500 uppercase tracking-tighter mb-2">Sale Price</label>
                                    <input type="number" id="purSalePrice" placeholder="Selling" class="w-full p-4 rounded-xl bg-appBg border-none font-bold text-gray-200 focus:ring-2 focus:ring-primary/20">
                                </div>
                            </div>
                        </div>
                        <button onclick="UI.hooks.addItemToPurchase()" class="w-full py-4 bg-sky-500 text-white rounded-2xl font-black active:scale-95 transition-all shadow-lg shadow-sky-900/40 mt-2">
                            ${i18n.t('addNew').toUpperCase()} TO LIST
                        </button>
            ...
            return `
                <div class="space-y-6 pb-40">
                    <div class="flex justify-between items-center bg-cardBg p-6 rounded-[2rem] border border-white/5 shadow-xl">
                        <div class="flex items-center gap-4">
                            <button onclick="router.navigate('dashboard')" class="w-10 h-10 rounded-full bg-white/5 flex items-center justify-center text-slate-400 hover:text-white transition-colors">
                                <i class="ri-arrow-left-line text-xl"></i>
                            </button>
                            <div>
        ...
        modal: (id, title, content) => `
            <div id="${id}" class="fixed inset-0 z-[100] hidden overflow-y-auto bg-slate-900/40 backdrop-blur-sm animate-in fade-in duration-200">
                <div class="min-h-screen px-4 flex items-center justify-center">
                    <div id="${id}-content" class="bg-cardBg w-full max-w-lg rounded-[2.5rem] shadow-2xl border border-white/5 overflow-hidden transform transition-all duration-300 scale-95 opacity-0">
                        ...
                        <div id="dropdown-${id}" class="absolute left-0 right-0 top-full mt-2 bg-cardBg border border-white/5 rounded-2xl shadow-2xl z-[150] max-h-[300px] overflow-y-auto hidden divide-y divide-white/5 backdrop-blur-xl">
                            ${options.map(opt => `
                                <div class="px-5 py-3.5 hover:bg-white/5 cursor-pointer flex flex-col transition-colors searchable-item" 
                                     ...
                                     onclick="UI.hooks.selectSearchItem('${id}', '${opt.value}', '${opt.label.replace(/'/g, "\\'")}', '${onSelectChange.replace(/'/g, "\\'")}')">
    ...
    hooks: {
        ...
        addItemToPurchase: () => {
            const name = document.getElementById('purProductName').value;
            const qty = Number(document.getElementById('purQty').value);
            const cost = Number(document.getElementById('purCost').value);
            const salePrice = Number(document.getElementById('purSalePrice').value);

            if (!name || qty <= 0 || cost <= 0) return UI.showToast('Details sahi se bharein', 'error');

            const products = Store.getProducts();
            const existingProduct = products.find(p => p.name.toLowerCase() === name.toLowerCase());

            UI.purchaseState.items.push({
                productId: existingProduct ? existingProduct.id : 'NEW_' + Date.now(),
                name: existingProduct ? existingProduct.name : name,
                qty,
                price: cost,
                sellingPrice: salePrice || (existingProduct ? existingProduct.price : 0),
                total: cost * qty,
                isNew: !existingProduct
            });

            // Clear inputs
            document.getElementById('purProductName').value = '';
        ...
        renderPurchaseCart: () => {
            ...
            const paid = UI.purchaseState.paid || 0;
            ...
            container.innerHTML = `
                <div class="space-y-4">
                    ...
                    <div class="p-5 bg-primary/5 rounded-2xl flex justify-between items-center">
                        <span class="font-bold text-slate-500">Total Purchase Bill:</span>
                        <span class="text-2xl font-black text-emerald-500">${UI.formatMoney(total)}</span>
                    </div>
        ...
        savePurchase: () => {
            ...
            const supplierName = document.getElementById('purSupplier').value;
            const total = UI.purchaseState.items.reduce((sum, i) => sum + i.total, 0);

            const purchase = {
                id: Date.now().toString(),
                supplier: supplierName || 'Cash Purchase',
                date: new Date().toISOString(),
            ...
            purchase.items.forEach(item => {
                ...
                if (prod) {
                    ...
                    if (item.sellingPrice > 0) prod.price = item.sellingPrice; 
                } else {
                    Store.addProduct({
                        name: item.name,
                        price: item.sellingPrice || item.price * 1.2, 
                        cost: item.price,
                        stock: item.qty,
                        unit: 'Pcs',
                        minStock: 5,
            ...
            Store.save(Store.KEYS.PRODUCTS, Store.getProducts());

            UI.showToast('Purchase Saved! Udhaar record ho gaya.');
            UI.purchaseState.items = [];
            UI.purchaseState.paid = 0;
        ...
        addItemToCart: () => {
            const nameInput = document.getElementById('itemSelectName');
            const qtyInput = document.getElementById('itemQty');
            const priceInput = document.getElementById('itemPrice');

            const productName = nameInput.value;
            if (!productName) return UI.showToast('Product name likhein', 'error');

            const products = Store.getProducts();
            const existingProd = products.find(p => p.name === productName);
            ...
            const existingInCart = UI.cartState.items.find(i => i.name === productName);
            if (existingInCart) {
                existingInCart.qty += qty;
                existingInCart.price = price;
                existingInCart.total = existingInCart.qty * price;
            } else {
                UI.cartState.items.push({ 
                    productId: existingProd ? existingProd.id : 'TEMP_' + Date.now(), 
                    name: productName, 
                    qty, 
                    price, 
                    total: qty * price 
        ...
        saveInvoice: () => {
            if (UI.cartState.items.length === 0) return UI.showToast('Cart khali hai!', 'error');
            ...
            const customerName = document.getElementById('saleCustomerName')?.value || 'Cash Sale';
            
            // Check if party exists by name
            let parties = Store.getParties();
            let party = parties.find(p => p.name.toLowerCase() === customerName.toLowerCase());
            
            // If name provided and due > 0, create party if not exists
            if (!party && customerName !== 'Cash Sale' && due > 0) {
                party = Store.addParty({ 
                    name: customerName, 
                    type: 'CUSTOMER', 
                    balance: 0 
                });
                parties = Store.getParties(); // Refresh list
            }
            ...
            // Update Stock for known products
            const products = Store.getProducts();
            ...
            // Update Party Balance
            if (party && due > 0) {
                // Find the latest reference in the list to update
                const allParties = Store.getParties();
                const partyToUpdate = allParties.find(p => p.id === party.id);
                if (partyToUpdate) {
                    partyToUpdate.balance += due;
                    Store.save(Store.KEYS.PARTIES, allParties);
                }
            }

            UI.showToast('Sale Complete! ' + (due > 0 ? 'Udhaar record ho gaya.' : 'Full payment received.'));
            UI.cartState.items = [];
        ...
        saveParty: () => {
            ...
            if (!name) return UI.showToast('Naam likhna zaroori hai', 'error');
        ...
        openQuickReceive: () => {
            ...
            UI.openModal('quickReceiveModal');
            document.getElementById('quickTotalDue').textContent = UI.formatMoney(party.balance);
            document.getElementById('quickAmount').value = party.balance;
            document.getElementById('quickAmount').focus();
        },

        processQuickReceive: () => {
            const amount = Number(document.getElementById('quickAmount').value);
            if (amount <= 0) return UI.showToast('Raqam sahi likhein', 'error');

            const partyId = UI.currentPartyId;
            const parties = Store.getParties();
            ...
            Store.save('hk_invoices', invoices);
            ...
            UI.showToast('Payment Jamma Ho Gayi!');
            
            // Refresh current page
            router.navigate('dues');
        },
        ...
        openQuickReceiveFromList: (id) => {
            UI.currentPartyId = id;
            UI.hooks.openQuickReceive();
        },
        ...
        viewPurchaseDetail: (id) => {
            ...
            const modalContent = `
                <div id="printablePurchase" class="p-6 bg-white text-black rounded-lg">
                    <div class="flex justify-between items-start border-b-2 border-slate-900 pb-4 mb-4">
                        <div>
                            <h2 class="text-xl font-black uppercase text-slate-900">${settings.businessName || 'Hisab Kitab'}</h2>
                            <p class="text-xs font-bold text-slate-500">PURCHASE VOUCHER</p>
                        </div>
                        <div class="text-right">
                            <p class="text-xs font-bold text-slate-500">#${pur.id.slice(-6)}</p>
                            <p class="text-xs font-medium">${new Date(pur.date).toLocaleDateString()}</p>
                        </div>
                    </div>

                    <div class="mb-4">
                        <div class="text-xs font-bold text-slate-400 mb-1 uppercase">Supplier</div>
                        <div class="text-sm font-black text-slate-800">${pur.supplier}</div>
                    </div>
                    ...
                    <div class="space-y-1 text-right">
                        <div class="flex justify-between text-lg font-black text-slate-900 pt-2 border-t">
                            ...
                            <span>${UI.formatMoney(pur.total)}</span>
                        </div>
                        <div class="flex justify-between text-xs font-bold text-emerald-600">
                            <span>Paid Amount:</span>
                            <span>${UI.formatMoney(pur.paid || 0)}</span>
                        </div>
                        ${(pur.total - (pur.paid || 0)) > 0 ? `
                            <div class="flex justify-between text-sm font-black text-red-600">
                                <span>Baqi Udhaar:</span>
                                <span>${UI.formatMoney(pur.total - (pur.paid || 0))}</span>
                            </div>
                        ` : ''}
                    </div>
                </div>
                ...
                <div class="mt-6 flex gap-3">
                    <button onclick="UI.hooks.copyBillToClipboard('printablePurchase')" class="flex-1 bg-emerald-600 text-white py-4 rounded-xl font-black shadow-lg flex items-center justify-center gap-2 active:scale-95 transition-all">
                        <i class="ri-file-copy-line text-xl"></i>
                        COPY BILL IMAGE
                    </button>
                    <button onclick="UI.closeModal('${modalId}')" class="w-14 bg-slate-800 text-white rounded-xl flex items-center justify-center shadow-lg active:scale-95 transition-all">
                        <i class="ri-close-line text-2xl"></i>
                    </button>
                </div>
            `;
            ...
            if (!document.getElementById(modalId)) {
                const modalDiv = document.createElement('div');
                modalDiv.innerHTML = UI.components.modal(modalId, 'Purchase Detail', '<div id="purchaseDetailContainer"></div>');
                document.body.appendChild(modalDiv.firstElementChild);
            }

            document.getElementById('purchaseDetailContainer').innerHTML = modalContent;
            UI.openModal(modalId);
        },

        copyBillToClipboard: (elementId) => {
            const element = document.getElementById(elementId);
            if (!element) return;
            
            if (typeof html2canvas === 'undefined') {
                return UI.showToast("System error: Image library missing", "error");
            }

            UI.showToast("Generating Image...");
                ...
                canvas.toBlob(blob => {
                    try {
                        ...
                        navigator.clipboard.write([item]).then(() => {
                            UI.showToast("✅ Bill Image Copied! Now Paste in Chat.");
                        }).catch(err => {
                            console.error(err);
                            UI.showToast("Clipboard error. Use screenshot.", "error");
                        });
                    } catch (e) {
                        UI.showToast("Browser doesn't support image copy.", "error");
                    }
                }, 'image/png');
            });
        },
        ...
        deletePurchase: (id) => {
            ...
            if (pur) {
                // Restore Stock: Subtract quantities added by this purchase
                const products = Store.getProducts();
                pur.items.forEach(item => {
                ...
                Store.save(Store.KEYS.PURCHASES, remaining);
                
                UI.showToast('Purchase Deleted & Stock Restored!');
                router.navigate('purchases');
            }
        },
